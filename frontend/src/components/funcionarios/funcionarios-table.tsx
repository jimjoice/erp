...
export function FuncionariosTable({ data, total, page, pageSize, isLoading, onPageChange, onEdit, onDelete }: Props) {
  ...
  const columns: ColumnDef<Funcionario>[] = [
    ...
    },
    {
      ...
      header: "Cargo",
      cell: ({ row }) => (
        <Badge variant="outline" className={CARGO_COLORS[row.original.cargo] ?? ""}>
          {row.original.cargo}
        </Badge>
      ),
    },
    ...
    },
    {
      ...
      header: "Salário",
      cell: ({ row }) =>
        row.original.salario != null ? brl(row.original.salario) : "—",
    },
    {
      ...
      cell: ({ row }) =>
        row.original.dataAdmissao
          ? new Date(row.original.dataAdmissao).toLocaleDateString("pt-BR")
          : "—",
    },
    {
      id: "acoes",
  ...
  return (
    <div className="space-y-3">
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={columns.length} className="h-32 text-center text-muted-foreground">Nenhum funcionário encontrado.</TableCell></TableRow>
            ) : (
      ...
      <div className="flex items-center justify-between text-sm text-muted-foreground">
        <span>{total === 0 ? "Nenhum resultado" : `${(page - 1) * pageSize + 1}–${Math.min(page * pageSize, total)} de ${total} funcionário${total !== 1 ? "s" : ""}`}</span>
